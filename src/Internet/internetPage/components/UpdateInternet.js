...
const AddInternet = (props) => {
    ...
    const {updateInternet,findInternetById} = internetStore;

    const [internet,setInternet]=useState(null)


    useEffect(async () => {
        findInternetById(params.id).then(res=>{
            setInternet(res.data)
            form.setFieldsValue({
                name: res.data?.name,
                ip:res.data?.ip,
                port:res.data?.port,
    ...
    const onFinish = async () => {
        ...
        await updateInternet({...values,id:internet.id}).then(res=>{
